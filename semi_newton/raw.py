...
def semi_newton_paso(fun: callable, grad_fun: callable, bk: callable,
    ...
    # Posicion inicial
    x0 = -wk / c  # Cota desigualdad <wk,dk> <=-C||dk||^2
    x0 = x0.append(rho)  # hat{x0} = [x0 rho_max]
    ...
    sol = minimize(fun=lambda x: norm(x[:-1]),  # ||d||^2
                   ...
                   method="trust-constr",
                   bounds=[Bounds(lb=1e-10) for _ in range(wk.shape[0])] + [Bounds(0, rho)],
                   )
    ...
    while True:
        ...
        tau[-2:] = tau[:-1]  # tau_k-1 = tau_k y tau_k-2 = tau_k-1
        tau_bar[-2:] = tau_bar[:-1]  # tau_bar_k-1 = tau_bar_k y tau_bar_k-2 = tau_bar_k-1
